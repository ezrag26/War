...
struct vtable_pt
{
    void (*dtor)( PublicTransport*);
    void (*display)(const PublicTransport*);
};
struct vtable_mb
{
    void (*dtor)( Minibus*);
    void (*display)(const Minibus*);
    void (*wash)(const Minibus*, int minuts);
};

struct vtable_taxi
{
    void (*dtor)( Taxi*);
    void (*display)(const Taxi*);
    void (*wash)(const Minibus*, int minuts); /* TODO: Taxi does not inherit this function, it is not a derived class of Minibus */
};

struct vtable_staxi
{
    void (*dtor)(SpecialTaxi*);
    void (*display)(const SpecialTaxi*);
    void (*wash)(const Minibus*, int minuts); /* TODO: Neither does SpecialTaxi inherit this function */
};
...
PublicTransport PublicTransportCCtor(PublicTransport *const pt_,
                                   const PublicTransport *const other_);
void PublicTransportDisplay(const PublicTransport *const pt_);
void PublicTransportPrintCount(void);
int PublicTransportGetId(const PublicTransport *const pt_);

void MinibusCtor(Minibus *const mb_);
Minibus MinibusCCtor(Minibus* mb_, const Minibus* other_);
void MinibusDtor(Minibus *const mb_);
void MinibusDisplay(const Minibus *const mb_);
void MinibusWash(const Minibus *const mb_, int minutes_);

void TaxiCtor(Taxi *const tx_);
Taxi TaxiCCtor(Taxi *const tx_, const Taxi *const other_);
void TaxiDtor(Taxi *tx_);
void TaxiDisplay(const Taxi *const tx_);

void SpecialTaxiCtor(SpecialTaxi *const st_);
void SpecialTaxiDtor(SpecialTaxi *const st_);
SpecialTaxi SpecialTaxiCCtor(SpecialTaxi *st_, const SpecialTaxi *other_);
void SpecialTaxiDisplay(const SpecialTaxi *const st_);
...
void PrintInfoPT(const PublicTransport *const m_);

/* TODO: same thing here */
void PrintInfoMB(const Minibus *const m_);
void PrintInfo(void);
int MaxFuncInt(int t1, int t2);
void GlobalTaxiDisplay(const Taxi s); /* TODO: I don't think const means anything here */

int g_publicTransportCount = 0;
VTableForPT g_vtableForPT = { PublicTransportCtor, PublicTransportDisplay };
VTableForMB g_vtableForMB = { MinibusDtor, MinibusDisplay, MinibusWash };
VTableForTaxi g_vtableForTaxi = { TaxiDtor, TaxiDisplay, MinibusWash };
VTableForSTaxi g_vtableForSTaxi = { SpecialTaxiDtor ,SpecialTaxiDisplay, MinibusWash };
...
Minibus MinibusCCtor(Minibus *const mb_, const Minibus *const other_)
{
...
Taxi TaxiCCtor(Taxi *const tx_, const Taxi *const other_)
{
    PublicTransportCCtor((PublicTransport*)(other_),
                (PublicTransport*)(tx_));
...
SpecialTaxi SpecialTaxiCCtor(SpecialTaxi *const st_, const SpecialTaxi *const other_)
{
    TaxiCCtor((Taxi*)(st_),
                (Taxi*)(other_));
...
void SpecialTaxiDisplay(const SpecialTaxi *const pt_)
{
    int id = PublicTransportGetId((PublicTransport *)pt_);
    printf("SpecialTaxi::display() ID:%d\n", id );
}

void PrintInfoPT(const PublicTransport *const pt_)
{
